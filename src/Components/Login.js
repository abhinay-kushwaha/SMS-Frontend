...
const Login = ({ onClose }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { token, role, id: studentId } = response.data;
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center items-center h-screen" style={{ backgroundImage: "linear-gradient(to top, #d9afd9 0%, #97d9e1 100%)" }}>
      <div className="bg-white shadow-2xl p-8 rounded-lg max-w-md w-full transform transition-transform duration-300 hover:scale-105">
        <Link to="/">
          <button className="text-2xl   absolute right-8 top-3 text-black ">x</button>
        </Link>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          <button type="submit" className="bg-purple-600 text-white font-semibold rounded-full py-2 mt-4 hover:bg-purple-700 transition-colors duration-300">
            Log in
          </button>
        </form>
        ...
        <div className="mt-6 text-center text-sm">
          ...
          <Link to="/signUp" className="text-purple-600 font-semibold hover:text-purple-800 transition-colors duration-300">
            Sign Up
          </Link>
        </div>
      </div>
