...
const SignUp = ({ onClose }) => {
  ...
  return (
    <div className='flex flex-col md:flex-row justify-center items-center h-screen' style={{ backgroundImage: 'linear-gradient(to top, #d9afd9 0%, #97d9e1 100%)' }}>
      <div className='bg-white shadow-2xl p-8 rounded-lg max-w-md w-full'>
        <p className='text-3xl ml-auto text-black cursor-pointer' onClick={onClose}>x</p>
        <h1 className='font-bold font-serif text-2xl text-center mb-6'>Create Account</h1>
        ...
        <form onSubmit={handleSignUp} className='flex flex-col gap-4'>
          ...
          <button
            type='submit'
            className='bg-purple-600 text-white font-semibold rounded-full py-2 mt-4 hover:bg-purple-700 transition-colors duration-300 focus:outline-none'
          >
            Create Account
          </button>
        </form>
        ...
        <div className='flex justify-center mt-4 text-sm'>
          <p>Already have an account? <Link to="/login" className='text-blue-400 font-semibold ml-1 hover:text-blue-600 transition-colors duration-300'>Log in</Link></p>
        </div>
      </div>
    </div>
  );
